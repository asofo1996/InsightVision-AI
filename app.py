...
prompt_text = st.text_area("🗨️ Ollama 분석 프롬프트", "Please analyze the content type, main audience, tone, and suggest 3 improvements.")
...
with st.expander("📁 Google Drive에서 이미지 선택"):
    ...
    if image_files:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
            ...
            st.image(image_obj, caption="Drive 이미지", use_container_width=True)
...
with col2:
    if video_path is not None:
        if st.button("🎬 영상 분석 시작"):
            with st.spinner("🎮 1초 단위 프레임 추출 중..."):
            ...
            with st.spinner("🧠 Ollama 종합 분석 및 솔루션 생성 중..."):
